...
class Habit(models.Model):
    ...
    associated_habit = models.ForeignKey('self', on_delete=models.SET_NULL, **NULLABLE,
                                         verbose_name='link to other habit')
...
class SenderDailyLog(models.Model):
    ...
    habit_id = models.OneToOneField(Habit, default=1, verbose_name='habit id',
                                    on_delete=models.CASCADE, primary_key=True)
    daily_status = models.CharField(max_length=6, choices=STATUS,
                                    default=CREATE, verbose_name='sending status')
